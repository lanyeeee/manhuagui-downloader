...
impl DownloadTask {
    ...
    async fn process(self) {
        let download_chapter_task = async {
            ...
            for (i, url) in img_urls.into_iter().enumerate() {
                ...
                let url = url.clone();
                // 创建下载任务
                join_set.spawn(self.clone().download_img(url, save_path));
            }
            // 等待所有下载任务完成
            ...
            if downloaded_img_count != total_img_count {
                let err_title =
                    format!("`{comic_title} - {group_name} - {chapter_title}`下载不完整");
                let err_msg = format!(
                    "总共有`{total_img_count}`张图片，但只下载了`{downloaded_img_count}`张"
                );
        ...
        loop {
            ...
            tokio::select! {
                ...
                _ = state_receiver.changed() => {
                    match self.chapter_handle_state_change(&mut permit, &mut state_receiver) {
                        ControlFlow::Continue(()) => continue,
                        ControlFlow::Break(()) => break,
                    }
                }
    ...
    fn chapter_handle_state_change<'a>(
        &'a self,
        permit: &mut Option<SemaphorePermit<'a>>,
    ...
    async fn download_img(self, url: String, save_path: PathBuf) {
        let download_img_task = async {
            let url = url.clone();
            let chapter_id = self.chapter_info.chapter_id;
            let comic_title = &self.chapter_info.comic_title;
            let group_name = &self.chapter_info.group_name;
        ...
        loop {
            ...
            tokio::select! {
                ...
                control_flow = self.acquire_img_permit(&url, &mut permit), if state_is_downloading && permit.is_none() => {
                    match control_flow {
                        ControlFlow::Continue(()) => continue,
                        ControlFlow::Break(()) => break,
                    }
                ...
                _ = state_receiver.changed() => {
                    match self.img_handle_state_change(&url, &mut permit, &mut state_receiver) {
                        ControlFlow::Continue(()) => continue,
                        ControlFlow::Break(()) => break,
                    }
                }
